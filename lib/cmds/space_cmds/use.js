...
export const desc = 'Sets the default space which every command will use when the --space-id option is not specified.'
...
function showSuccess (space) {
  success(`Now using Space ${highlightStyle(space.name)} (${highlightStyle(space.sys.id)}) as the default option. The default space can be overridden by specifying the \`--space-id\` parameter.`)
}
